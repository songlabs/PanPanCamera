...
enum LocalSkinCorrection {
    ...
    static func effectiveMask(source: CIImage, regions: [FaceRegion],
        ...
        // including thin curved strokes whose feathered peak is below 0.55.
        let strictFeatures = try CoreImageRendering.grayMask(masks.featureProtectionMask,
                                                             scale: 1 / 0.30)
        return try multiply(masks.effectiveSkinMask,
                            CoreImageRendering.grayMask(strictFeatures, scale: -1, bias: 1))
    }
}
...
struct BlemishAttenuationStep: Sendable {
    func makeOutput(source: CIImage, regions: [FaceRegion], landmarks: [FacialLandmarks],
        ...
        // Do not guess feature exclusions from a bounding box when tracking fails.
        let covered = regions.filter { region in
            landmarks.contains { face in
                face.region == region && FacialLandmarkRegion.protectedFeatures.allSatisfy {
                    !(face.features[$0] ?? []).isEmpty
                }
            }
        }
        guard let faceMask = try SoftFaceMaskGenerator().makeMask(regions: covered, in: source.extent),
              let side = covered.map({ min($0.imageRect(in: source.extent).width,
                                            $0.imageRect(in: source.extent).height) }).min()
        else { return nil }
        ...
        let allowed = try LocalSkinCorrection.multiply(effectiveSkinMask, faceMask)
...
struct UnderEyeRegion: Equatable, Sendable {
    ...
    static func make(landmarks: FacialLandmarks, eye: FacialLandmarkRegion,
        ...
        for face in landmarks where regions.contains(face.region) {
            for eye in [FacialLandmarkRegion.leftEye, .rightEye] {
                guard let area = UnderEyeRegion.make(landmarks: face, eye: eye, in: source.extent),
                      !used.contains(area),
                      let faceMask = try SoftFaceMaskGenerator().makeMask(regions: [face.region], in: source.extent)
                else { continue }
                used.append(area)
                let allowed = try LocalSkinCorrection.multiply(faceMask, effectiveSkinMask)
