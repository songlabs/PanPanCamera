...
/// Uses the shared normalized dense landmark regions. Only geometry masks
...
final class MakeupProcessingStep: @unchecked Sendable {
    ...
    private var cachedFaces: [GeometryKey] = []
    ...
    func makeOutput(source: CIImage, faces: [AnalyzedFace],
    ...
    private func geometryMasks(faces: [AnalyzedFace], extent: CGRect,
                               components: [Component]) throws ->
        (masks: [Component: CIImage], brows: [(mask: CIImage, side: CGFloat)]) {
        lock.lock(); defer { lock.unlock() }
        let keys = faces.map(GeometryKey.init)
        if keys != cachedFaces || extent != cachedExtent {
            cachedFaces = keys
            cachedExtent = extent
            cachedMasks = [:]
            cachedComponents = []
    ...
    private func faceMask(_ face: AnalyzedFace, component: Component, extent: CGRect,
