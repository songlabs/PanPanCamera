...
struct FeatureProtectionMaskGenerator: Sendable {
    ...
    func makeMask(faces: [AnalyzedFace], in extent: CGRect) throws -> CIImage? {
        ...
        // Identity and geometry travel together; every face contributes protection.
        for face in faces {
            let rect = FaceAnalysisCoordinates.bounds(FaceAnalysisCoordinates.corners(face.boundingBox)
                .map { CGPoint(x: extent.minX + $0.x * extent.width, y: extent.minY + $0.y * extent.height) })
            guard !rect.isNull, rect.width >= 1, rect.height >= 1 else { continue }
            for feature in Self.protectedFeatures {
                let points = face.landmarks.imagePoints(for: feature, in: extent)
                guard !points.isEmpty else { continue }
                let policy = Policy.feature(feature)
                let side = min(rect.width, rect.height)
                guard let raster = rasterize(points, polygon: Self.polygons.contains(feature),
                                             expansion: side * policy.expansion, feather: side * policy.feather) else {
                    // Never proceed with an unprotected feature after allocation failure.
                    throw CoreImageRendering.Failure.renderFailed
                }
    ...
    private static let protectedFeatures: [FacialLandmarkRegion] = [
        .leftEye, .rightEye, .leftEyebrow, .rightEyebrow, .outerLips, .innerLips, .nose
    ]
    private static let polygons: Set<FacialLandmarkRegion> = [.leftEye, .rightEye, .outerLips, .innerLips]
