...
## Next-stage module plan — documentation only
...
FaceTracking/                 planned, no production Swift implementation yet
├── VisionFaceDetector
├── FaceLandmarkProvider
└── FaceTracker
```

Presentation owns Views and localization. Application coordinates camera activity and processing state. Camera owns acquisition and capture-graph control. FaceTracking will consume an explicit frame/orientation contract and produce face/landmark/tracking results. BeautyEngine will interpret parameter snapshots and tracking inputs. Rendering will own image/GPU rendering, including a future BeautyRenderer and MetalPipeline. Domain holds UI-independent value contracts. Do not introduce Camera → Presentation, FaceTracking → SwiftUI or BeautyEngine → SwiftUI dependencies.

The UI's independent 0–100 values can later map to normalized 0.0–1.0 engine inputs; UI selection and the default value of 50 do not define an algorithm's neutral value. No engine or frame API is invented in this repair. The sequence is Vision Face Detection, Face Landmarks, Stable Face Tracking, BeautyEngine input model, then Metal rendering.

The 0.1 scope guard in `scripts/check_project.py` continues to reject Vision, Metal, CoreImage and AVCaptureVideoDataOutput in production. Update the guard deliberately when implementing the corresponding later phase; do not remove it ahead of that work.
