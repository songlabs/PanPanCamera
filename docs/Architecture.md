...
Skin and face values start at 50, clamp to 0–100, and reject non-finite inputs. Each Auto control batch-writes its category's concrete values; a later concrete adjustment remains independent and does not change Auto or its siblings. `BeautyConfiguration` retains the normalized Auto values as UI state but processing uses each normalized concrete value directly, with no second multiplication. No implemented concrete effect is an exact branch-specific bypass. Smoothing uses the existing texture reconstruction plus landmark/edge protection, brightening is a bounded local face-mask lift, tone uses the existing neutral luminance-consistency pass, and Preview Face Correction uses contour/eyebrow-driven local displacement.
